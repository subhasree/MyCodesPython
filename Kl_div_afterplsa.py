...
def find_probabilites(fileN):
    ...
    for l in lines:
        ...
        expressions = l.split(' ')
        if (expressions[0] != ''):
           # print "Expression is:", expressions[2]
            probabilities.append(expressions[2])
    print probabilities
...
def find_kl_write_csv(d, files):
    files = files.replace("words.txt", "")
    write_file = './all_plsi/' + files +'.csv'
    with open(write_file, 'a') as f_text:
#        writer = csv.writer(f_csv)
        f_text.write("Videos, KL \n")
        path = '/home/sbasu/VideoClustering/PLSA_recal/VideoPLSI/'
        for filename in os.listdir(path):
            file_open = path + filename
            #file_open = "/home/sbasu/Videopedia_MM/Video_justPLSI/_1RFDkYS2QQ_words.txt"
            f = open(file_open, 'r')
            text = f.read()
            try:
...
path_wiki = '/home/sbasu/VideoClustering/PLSA_recal/VideoPLSI/'
