...
##W = px.load_workbook('./allWords.xlsx', use_iterators = True)
##p = W.get_sheet_by_name(name = 'list')
##
...
text_srt = [[word for word in item.lower().split() if word not in stoplist] for item in words.split(" ")]

#print text_srt
print len(text_srt)


for list_word in text_srt:
    try:
        acad_words = acad_words + " " + list_word[0]
    except IndexError:
